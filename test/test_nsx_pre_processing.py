...
def test_pre_process_note_page(note_1, attachments, raw_content, output_regex):
    ...
    match = re.findall(output_regex, result)
...
def test_test_pre_process_note_page_obsidian_output(note_1):
    ...
    output_regex = r"""<head><title>Page 1 title</title><meta title="Page 1 title"/></head><p>Below is an image of the design of the line chart as seen in note-station</p><p><p>\d{15}\</p></p><p>Some more text&lt;/div</p>"""
    note_1.pre_process_content()
...
def test_get_image_relative_path_force_excption_and_confirm_is_handled(note_1):
    attachments = Attachments('not_going_to_match', Path('myfile.txt'))
    note_1._attachments = {'an_attachment': attachments}
    pre_processor = NoteStationPreProcessing(note_1)
